...
import { mapCardToDisplay } from './utils/cardUtils';
...
export function CardsScreen() {
  ...
  const handleCreateCardOfType = useCallback(
    async (type: PaymentCardType) => {
      try {
        const cardholderName = account?.ownerName || 'Portador';
        const cardType: CardType = type === 'CREDIT' ? 'CREDIT' : 'DEBIT';
        const createdCard = await createCard(cardType, cardholderName);
